...
class NavTest():  
    def __init__(self):  
        ...
        # 保存机器人的在rviz中的初始位置  
	    # 保存当前的估计的时间和位姿  header  pose   初始化时的位置
        # 定义initial_pose
        initial_pose = PoseWithCovarianceStamped()  

        # 保存成功率、运行时间、和距离的变量  
        n_locations = len(locations)  # n_locations = 5
        n_goals = 0  
        n_successes = 0  
        i = n_locations  
        distance_traveled = 0  
        start_time = rospy.Time.now()    #从现在开始计时
        running_time = 0  
        location = ""  
        last_location = ""    
        ...
        # 如果已经走完了所有点，再重新开始排序 
        # 第一次执行训话时也满足 i = lications
	    #  sample(seq, n) 从序列seq中选择n个随机且独立的元素；
            if i == n_locations:  
                i = 0  
                # 从list中抽取n_locations个点重新排列成为sequence
                sequence = sample(list(locations), n_locations)  
 
                # 如果上一次循环的最后一个点和新的排列中第一个点相同，则跳过  
                # 第一次执行时，lastlocation = "" 不满足 (i = 0)
